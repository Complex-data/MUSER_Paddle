...
def tokenize(text, max_length, tokenizer, second_text=None):
    if second_text is None:
        ...
        # PaddlePaddle
        text = tokenizer.encode(text.replace('[^\w\s]+|\n', ''))
        second_text = tokenizer.encode(second_text.replace('[^\w\s]+|\n', ''))
    ...
    sentence_converted = encoded_inputs['input_ids']

    sentence_mask = encoded_inputs['attention_mask']
    ...
    sentence_segment = [0] * len(text) + [1] * len(second_text)
