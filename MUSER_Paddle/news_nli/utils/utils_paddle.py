def get_pair_input(tokenizer, sent1, sent2, model_type):
    ...
    #indexed_tokens = tokenizer.encode(text, return_dict=False)[1:-1]
    encoded_result = tokenizer.encode(text, return_dict=True)
    indexed_tokens = encoded_result['input_ids'][1:-1]
